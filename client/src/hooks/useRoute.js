...
export default function useRoute() {
  ...
  // Toggle block on an edge
  const handleBlockEdge = useCallback(({ from, to }) => {
    setBlockedEdges(prev => {
      const exists = prev.some(b => b.from === from && b.to === to);
      if (exists) return prev.filter(b => !(b.from === from && b.to === to));
      return [...prev, { from, to }];
    });
  }, []);
